...
class MyoWebBluetooth{
  ...
  handleIMUDataChanged(event){
    ...
    let roll = Math.atan2(2.0*(orientationW * orientationX + orientationY * orientationZ),1.0-2.0*(orientationX * orientationX + orientationY * orientationY));
    let pitch = Math.asin(Math.max(-1.0, Math.min(1.0, 2.0*(orientationW * orientationY - orientationZ * orientationX))));
    let yaw = Math.atan2(2.0*(orientationW * orientationZ + orientationX * orientationY), 1.0-2.0*(orientationY * orientationY + orientationZ * orientationZ));
